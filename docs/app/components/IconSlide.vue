...
const { data: icons, refresh: refreshIcons } = await useAsyncData('icons', async () => {
...
const { pause: pauseInterval, resume: resumeInterval } = useIntervalFn(refreshIcons, 2000)
  ...
  >
    <UiIcon
      :name="{
        ...
        'clipboard-check': copiedText.includes(`'${icon}'`) && copied,
      }"
      :size="20"
    />
  </button>
